...
Config LoadConfig(const std::wstring& path) {
    ...
    for (const std::string& rawLine : SplitChar(content, '\n')) {
        ...
        } else if (key == "project_list_fixed_match_taskbar") {
            cfg.projectListFixedMatchTaskbar = (val == "1" || _stricmp(val.c_str(), "true") == 0);
    ...
    Log(L"config: loaded thickness=%d opacity=%d rescan_ms=%d show_label=%d show_project_list=%d project_list_style=%ls project_list_position=%ls project_list_fixed_match_taskbar=%d project_list_order=%ls project_list_opacity_normal=%d project_list_opacity_hover=%d project_list_activate_on_hover=%d label_height=%d label_font_size=%d label_text_color_auto=%d label_text_color=%06X label_alias_format=[%ls] verbose_logging=%d colors=%zu ai_indicator_enabled=%d ai_indicator_provider=%hs",
        ...
        cfg.projectListFixed ? L"fixed" : L"free", cfg.projectListFixedMatchTaskbar,
